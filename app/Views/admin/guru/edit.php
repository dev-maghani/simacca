...
<div class="bg-white rounded-xl shadow p-6">
    ...
    <!-- Form -->
    <form action="<?= base_url('admin/guru/update/' . $guru['id']) ?>" method="POST" id="guruForm">
        <?= csrf_field() ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div class="form-section rounded-xl p-6 text-white">
                ...
                <div class="space-y-4">
                    ...
                    <!-- Password -->
                    <div>
                        <label class="block text-sm font-medium mb-1">Password (Biarkan kosong jika tidak diubah)</label>
                    ...
                    <div>
                        ...
                        <select name="role" 
                            ...
                            <option value="guru_mapel" <?= old('role', $userData['role']) == 'guru_mapel' ? 'selected' : '' ?>>Guru Mata Pelajaran</option>
                            <option value="wali_kelas" <?= old('role', $userData['role']) == 'wali_kelas' ? 'selected' : '' ?>>Wali Kelas</option>
                        </select>
            ...
            <div class="border border-gray-200 rounded-xl p-6">
                ...
                <div class="space-y-4">
                        ...
                        <div class="flex space-x-4">
                            ...
                            <label class="inline-flex items-center">
                                <input type="radio" name="jenis_kelamin" value="P" 
                                       class="text-indigo-600 focus:ring-indigo-500"
                                       <?= old('jenis_kelamin', $guru['jenis_kelamin']) == 'P' ? 'checked' : '' ?> required>
                    ...
                    <div>
                        ...
                        <select name="mata_pelajaran_id" 
                            ...
                            <?php foreach ($mapelList as $id => $mapel): ?>
                                <option value="<?= $id ?>" <?= old('mata_pelajaran_id', $guru['mata_pelajaran_id']) == $id ? 'selected' : '' ?>>
                                    <?= esc($mapel) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <!-- Wali Kelas Section -->
                    <div id="waliKelasSection" class="border-t pt-4 <?= old('role', $userData['role']) != 'wali_kelas' ? 'hidden' : '' ?>">
                        <label class="inline-flex items-center">
                        ...
                        <!-- Kelas Selection -->
                        <div id="kelasSelection" class="mt-3 <?= !old('is_wali_kelas', $guru['is_wali_kelas']) ? 'hidden' : '' ?>">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Pilih Kelas</label>
                            <select name="kelas_id" 
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                <option value="">Pilih Kelas</option>
                                <?php foreach ($kelasList as $id => $kelas): ?>
                                    <option value="<?= $id ?>" <?= old('kelas_id', $guru['kelas_id']) == $id ? 'selected' : '' ?>>
                                        <?= esc($kelas) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
